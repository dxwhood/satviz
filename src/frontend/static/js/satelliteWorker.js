...
//~~~~~~~~~~~~~~~~~~~~ PASTE OF SATELLIT.JS TO AVOID IMPORT ERRORS ~~~~~~~~~~~~~~~~~~~~
/*!
 * satellite-js v5.0.0
...
function propagateAllSatellites(epoch = new Date()) {
    for (let i = 0; i < sats.length; i++) {
        const position = get_sat_ecef(sats[i], epoch);
        if (isFinite(position.x) && isFinite(position.y) && isFinite(position.z)) {
            sats[i]['position'] = position;
        } else {
            sats[i]['position'] = { x: 0, y: 0, z: 0 };
        }
    }
    return sats;
}

function get_sat_ecef(sat, epoch = new Date()) {
    let positionAndVelocity = propagate(sat.satrec, epoch);
    let positionEci = positionAndVelocity.position || { x: 0, y: 0, z: 0 };
    let gmst = gstime(epoch);
    return eciToEcf(positionEci, gmst);
}
...
onmessage = function(e) {
    if (e.data.type === 'initialize') {
        ...
        const updatedSats = propagateAllSatellites(epoch);
